...
class VManager():
    def __init__(self, vprocesses, vconnections, vlocks=[], vselects=[], logging=False, output='output/Tick', outputFormat='pdf', interactive=False, logfilename="log.txt") -> None:
    ...
    def stepwise_ticks(self, processes):
        ...
        while running:
            ...
            if response == "q" or response == "quit":
                ...
                vmanager.log.close()
            if running:
                processes = vmanager.run_all_to_tick(processes)
    ...
    def run_all_to_tick(self, processes):
        ...
        if self.previousTickProcessCount == 0 and len(processes) == 0:
            print("!!!DEADLOCK OR BUG!!! - there has been no able processes two ticks in a row. Therefore the execution will not progress")
            return False
        ...
        updateLockList, updateNodesList, updateEdgesList = self.handleRequests(updateLockList) # Split this?
...
def selectSharedLockTwoLockedProducersTwoConsumers(conn1, conn2, conn3, conn4, lock1, lock2, vs):
    while True:
        data1 = None
        data2 = None
...
def selectVariableProducersConsumers(in_connections, out_connections, vs):
    for i in range(10):
        (inputs, _, _) = vs.select(in_connections, [], [])
...
def selectSharedLockTwoLockedProducersTwoConsumersTest():
...
def ComplexTest1():
    ...
    processes = [
            ...
            VProcess(
                target=selectSharedLockTwoLockedProducersTwoConsumers, 
                args=(c1, c2, p5, p6, vlock11, vlock51, vs1),
                name="DS1"
            ),
            VProcess(
                target=selectSharedLockTwoLockedProducersTwoConsumers, 
                args=(c3, c4, p7, p8, vlock41, vlock52, vs2),
                name="DS2"
            ),
            VProcess(
                target=selectSharedLockTwoLockedProducersTwoConsumers, 
                args=(c6, c7, p9, p10, vlock12, vlock42, vs3),
                name="DS3"
            ),
            VProcess(
                target=consumer, 
...
def selectMultipleProducersConsumers():
    ...
    processes = [
            ...
            VProcess(
                target=selectVariableProducersConsumers, 
                args=(in_connections, out_connections, vs),
                name="DS"
            ),
            VProcess(
                target=consumer, 
...
# Test 3
#processes, channels, locks, selects = selectSharedLockTwoLockedProducersTwoConsumersTest()

# Test 4
#processes, channels, locks, selects = selectMultipleProducersConsumers()

# Test 5
processes, channels, locks, selects = ComplexTest1()

vmanager = VManager(processes, channels, locks, selects, outputFormat="png", interactive=True)

vmanager.init_graph()
vmanager.start()

vmanager.stepwise_ticks(processes)