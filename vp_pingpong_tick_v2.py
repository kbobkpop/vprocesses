...
class PatchManager():
    ...
    def run_all_to_tick(self):
        ...
        while True:
            ...
            #time.sleep(1)
            (inputs,_,_) = select.select(self.requests_from_worker_connections, [], [], 10000)
            for from_worker_channel in self.requests_from_worker_connections:
            ...
            for c in connections:
                c.send(True)
            print("sendEdges to be updated:", sendEdges)
            for s in sendEdges:
                self.updateEdges(s[0], s[1], s[2])
            self.drawGraph()
            print("recvEdges to be updated:", recvEdges)
            for r in recvEdges:
...
class PatchConnection():
    ...
    def recv(self):
        print(f"{multiprocessing.current_process()} receiving")
    ...
    def send(self, data):
        print(f"{multiprocessing.current_process()} sending")
...
#Do I really need the name, if I can compare the object? Identical objects will not be equal because of different addess space for forked process, but I could just generate a name.
def PatchPipe(name=""):
