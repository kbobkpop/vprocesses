...
class VManager():
    ...
    def runAllToTick(self, processes):
        ...
        #updateSelectsList = self.handleSelect()

        updateNodesList, updateEdgesList = self.handleReceive(updateEdgesList, updateNodesList)

        #self.updateGraph(updateLocksList, updateNodesList, updateEdgesList, updateSelectsList)
        self.updateGraph(updateLocksList, updateNodesList, updateEdgesList)
    ...
    def parseRequest(self, conn, request, process):
        action, data = request 
    ...
    def getRequests(self, ableProcesses):
        ...
        #for index, process in enumerate(ableProcesses):
            ...
            for conn in inputs:
                request = conn.recv()
                self.parseRequest(conn, request, process)
                requestsSentFromAbleProcesses[index] += 1
    ...
    def handleSelect(self, updateNodesList):
        ...
        for select in self.waitingToSelect:
            updateNodesList.append([select[1], 'red', 'dashed', select[1]])
        ...
        for pslct in tmpSelectList:  # self.waitingToSelect #Select objec
            ...
            if remove == True:
                pslct[2].send(True)
                process = self.getProcess(pslct[1])
    ...
    def acquireLocks(self, updateLocksList, updateNodesList, progressList):
            ...
            if lock not in self.lockedLocks:
                ...
                if len(templist) > 0:
                    if self.interactive and len(templist) > 1:
                        ...
                        #request = inputs[0].recv()
                        #print("received post lock request: ", request)
        ...
        return updateLocksList, updateNodesList
    ...
    def releaseLocks(self):
        ...
        return updateLockList, updateNodesList, progressList
...
#processes, channels = twoXProducerConsumerTest(4)
...
# Test 7
processes, channels, locks, selects = complexTest(5, outputdir)
